...
<br />
<div align="center">   <span class="link">Section II </span><br />     
...
FROM `gen_result` WHERE `result_id` = '.$V['result_id'].' AND `section_id` = '.$V['section_id'].' GROUP BY `section_sub_id`
			...
			if( isset($sectionMarks[0]) ){
				...
				foreach($sectionMarks as $K1=>$V1){
					...
					$query = $this->db->query('SELECT * FROM `section_sub` WHERE `pid` = "'.$V1['section_sub_id'].'"');

					$info1 = $query->row_array();

					if( $info1!=false ){

						$list[] = '<set name="'.$info1['title'].'" value="'.$V1['score'].'" />';
					...
					<tr>
						...
						<td bgcolor="#ffffff">'.( $info1!=false ? $info1['title'] : '' ).'</td>

						<td bgcolor="#ffffff"><div align="center">'.ceil($V1['score']).'</div></td>
	...
	?>
	<tr>
		<td bgcolor="#c00000" class="textwhite2"><?php echo $info!=false ? $info['title'] : ''; ?></td>
		<td bgcolor="#1f497d" class="textwhite2"><div align="center"><?php echo number_format($scoreTotal/$scoreCount, 2); ?></div></td>
	</tr>
	<?php
			echo implode(' ', $htmlSectionMarks);
		}
	}
	?>
