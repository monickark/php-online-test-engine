...
class member_model extends CI_Model {
	...
	if ($slug>="1")
		...
		$this->db->where('u_id',$slug);
		//$this->db->join('batch', 'user_details.batch_id = batch.batch_id and u_id = '.$slug );
		$query = $this->db->get();
		...
		$this->db->from('user_details');
		$this->db->where('u_id',$slug);
		//$this->db->join('batch', 'user_details = batch.batch_id order by u_id DESC LIMIT '.$limit.', 30 ');
		$query = $this->db->get();
		return $query->result_array();
